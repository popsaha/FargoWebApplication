window.globalCounter = 1; var ClientDetails = [];
$(function () {
    RegDdlClientChangeEvt();
    RegBtnResetEvt();
    ClientList = JSON.parse($("#hdnClientList").val());
    BindClien(ClientList);
    QuotaionProfa = JSON.parse($("#hdnQuotaionProfa").val());
    $("#txtQUOTATION_PROF_NO").val(QuotaionProfa);
    ...
    $("#ddlClientName").change(function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                BindClientPackageDetails(response);
...
function BindClientPackageDetails(details) {
    var body = ""; var k = 0;
    $("#tblGenerateQuotationProforma tbody").empty();
    $.each(details, function (i, item) {
        body += "<tr>";
        body += "<td width='5%'>" + window.globalCounter + "</td>";
        body += "<td width='35%'><input type='text' id='txtParticularName" + window.globalCounter + "' class='form-control txtParticularName' value='" + item.PACKAGE_NAME + "'/></td>";
        body += "<td width='25%'><input type='number' min='0' onKeyDown='if (this.value.length == 7) return false;' oninput='this.value =!!this.value && Math.abs(this.value) >= 0 ? Math.abs(this.value) : null' id='txtAmount" + window.globalCounter + "' class='form-control txtAmount' value='" + item.PRICE_WO_GST + "'/></td>";
        ...
        body += "<td width='10%'>No Action</td>";
        body += "</tr>";
        window.globalCounter++;
    });
    $("#tblGenerateQuotationProforma tbody").append(body);
    $("#hdnProforma").show();
}
function btnAddRow() {
    debugger
    var body = "";
    var i = $("#tblGenerateQuotationProforma tbody tr").length;
    body += "<tr>";
    body += "<td width='5%'>" + window.globalCounter + "</td>";
    ...
    body += "<td width='25%'><select class='form-control ddlIdtaxable select2' id='ddlIdtaxable" + window.globalCounter + "'><option value='1'>Taxable</option><option value='0'>Non-Taxable</option></td>";
    body += "<td width='10%'><span class='delete'><i class='fa fa-trash' aria-hidden='true' style='font-size:18px; cursor:pointer'></i></span></td>";
    body += "</tr>";
    $("#tblGenerateQuotationProforma tbody").append(body);
    window.globalCounter++;
}
function RegDeleteTblRow() {
    $("#tblGenerateQuotationProforma").on("click", ".delete", function (ev) {
        var $currentTableRow = $(ev.currentTarget).parents('tr')[0];
        $currentTableRow.remove();
        window.globalCounter--;
        $("#tblGenerateQuotationProforma tbody").find('tr').each(function (index) {
...
function ShowQuotation() {
    ...
    $("#lblClientName").text(ClientDetails[0].ORGNIZATIO_NAME);
    $("#lblClientAddress").text(ClientDetails[0].PRIMARY_ADDRESS);
    $("#lblClientState").text(ClientDetails[0].STATE_NAME);
    $("#lblClientGSTIN").text(ClientDetails[0].GST_NO);
    $("#lblClientPAN").text(ClientDetails[0].PAN_NO);
    $("#lblApplicationURL").text(ClientDetails[0].CLIENT_APPLICATION_URL);
    $("#lblClientContactNo").text(ClientDetails[0].PRIMARY_CONTACT_NO);
    var array = [];

    var QuotationRowlist = $("#tblGenerateQuotationProforma tbody tr").length;
    ...
    $.each(array, function (i, item) {
        ...
        if (item.Is_Taxable == "1") {
            MiddleString += "<tr> \
                <td style='font-size:12px;border-left:1px solid #000;border-right:1px solid #000;font-size:13px;'>&nbsp;&nbsp;<span style='font-weight:bold;'>" + item.Particular + "</span></td> \
                <td style='text-align:center;font-size:12px;border-right:1px solid #000;'><span style='font-weight:bold;'></span></td> \
                <td style='text-align:right;font-size:12px;border-right:1px solid #000;'><span id='lblAmount' style='font-weight:bold;margin-right:2px;font-size:14px'>" + inrFormat(parseFloat(item.Amount)) + "</span></td> \
                </tr>";
            otherTaxableValue += parseFloat(item.Amount);
        }
        else {
            MiddleString += "<tr> \
                <td style='font-size:12px;border-left:1px solid #000;border-right:1px solid #000;font-size:13px;'>&nbsp;&nbsp;<span>" + item.Particular + "</span></td> \
                <td style='text-align:center;font-size:12px;border-right:1px solid #000;'><span style='font-weight:bold;'></span></td> \
                <td style='text-align:right;font-size:12px;border-right:1px solid #000;'><span id='lblAmount' style='margin-right:2px;font-size:14px;'>" + inrFormat(parseFloat(item.Amount)) + "</span></td> \
                </tr>";
            otherNon_TaxableValue += parseFloat(item.Amount);
        }
    });
    var TaxableValue = (parseFloat(otherTaxableValue)) * parseFloat(ClientDetails[0].GST_PERCENT) / 100;
    var Amount_decimal = parseFloat(otherTaxableValue).toFixed(2);
    if (ClientDetails[0].STATE_NAME == "Maharashtra") {
        var halfRate = parseFloat(ClientDetails[0].GST_PERCENT) / 2;
        var halfTaxableValue = TaxableValue / 2;

        LastString = "<tr id='rowSGST'> \
                                 <td style='text-align:right;font-size:13px;border-left:1px solid #000;border-right:1px solid #000;font-weight:bold;'>Output SGST @ <span id='lblSGSTaxRate' style='margin-right:2px;' >" + halfRate + " %</span></td> \
                                 <td style='text-align:right;font-size:13px;border-right:1px solid #000;'>&nbsp;</td> \
                                 <td style='text-align:right;font-size:13px;border-right:1px solid #000;'><span id='lblSGSTAmount' style='font-weight:bold;margin-right:2px;'>" + inrFormat(halfTaxableValue.toFixed(2)) + "</span></td> \
                              </tr> \
                              <tr id='rowCGST'> \
                                 <td style='text-align:right;font-size:13px;border-left:1px solid #000;border-right:1px solid #000;font-weight:bold;'>Output CGST @ <span id='lblCGSTaxRate' style='margin-right:2px;'>" + halfRate + " %</span></td> \
                                 <td style='text-align:right;font-size:13px;border-right:1px solid #000;'>&nbsp;</td> \
                                 <td style='text-align:right;font-size:13px;border-right:1px solid #000;'><span id='lblCGSTAmount' style='font-weight:bold;margin-right:2px;'>" + inrFormat(halfTaxableValue.toFixed(2)) + "</span></td> \
                              </tr> \
                              <tr style='border-bottom:1px solid #000;'> \
                                 <td style='text-align:right;font-size:13px;border-left:1px solid #000;border-right:1px solid #000;font-weight:bold;'>&nbsp;</td> \
                                 <td style='text-align:right;font-size:13px;border-right:1px solid #000;'>&nbsp;</td> \
                                 <td style='text-align:right;font-size:13px;border-right:1px solid #000;'>&nbsp;</td> \
                              </tr>";
        TotalAmountWithTax = parseFloat(otherTaxableValue) + parseFloat(halfTaxableValue) + parseFloat(halfTaxableValue);

        var fullServiceGstAmt = parseFloat(otherTaxableValue) * parseFloat(ClientDetails[0].GST_PERCENT) / 100;
        ...
        if (QuotationRowlist != 0) {
            $.each(array, function (i, item) {
                if (item.Is_Taxable == "1") {
                    var fullTaxAmt = parseFloat(item.Amount) * parseFloat(ClientDetails[0].GST_PERCENT) / 100;
    ...
    var roundOff = Math.round(TotalAmountWithTax);
...
function GetQuotationProformaModel() {
    var model = {
        "QPID": "QPID",
...
function RegBtnSubmitEvt() {
    $("#btnSubmit").click(function () {
        ...
        var spinner = $('#loader');
        spinner.show();
        var model = GetQuotationProformaModel();
        ...
        model.GST_AMOUNT = ((_CalTotalGST_Amt * ClientDetails[0].GST_PERCENT) / 100);
        model.TOTAL_AMOUNT = model.GST_AMOUNT + model.PRICE_WO_GST;
        model.GST_PERCENTAGE = ClientDetails[0].GST_PERCENT;
